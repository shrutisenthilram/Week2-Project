...
class BoardController: NSObject,
    ...
    func resetBoardWithCurrentSettings(settings: [String: Any]) {
        ...
        if let numLetters = settings[kNumLettersKey] as? Int {
            ...
            print("NUmber of letters set to \(numItemsPerRow)")
            
        }
        else{
            print("Failed to get numLetters from settings")
        }
    }
    ...
    private func applyNumGuessesSettings(with settings: [String: Any]) {
        ...
        if let numGuesses = settings[kNumGuessesKey] as? Int {
            numRows = numGuesses
            print("Number of guesses (rows) ser to: \(numRows)")
        }
        else{
            print ("Failed to get numGuesses from settings")
        }
        // END YOUR CODE HERE
    }
    ...
    private func applyThemeSettings(with settings: [String: Any]) {
        ...
        let theme = WordTheme(rawValue: themeString){
            ...
            print("Theme applied: \(themeString), new goal word: \(goalWord)")
        }
        else{
            print("Failed to get or convert theme from settings")
        }
        // START YOUR CODE HERE
        // ...
        // END YOUR CODE HERE
    ...
    private func applyIsAlienWordleSettings(with settings: [String: Any]) {
        ...
        if let isAlienWordle = settings[kIsAlienWordleKey] as? Bool{
            ...
            print("Alien Wordle mode is now \(isAlienWordle ? "enabled" : "disabled")")
        } else {
            print("Failed to get Alien Wordle setting from settings")
        }
        // START YOUR CODE HERE
    }
}
